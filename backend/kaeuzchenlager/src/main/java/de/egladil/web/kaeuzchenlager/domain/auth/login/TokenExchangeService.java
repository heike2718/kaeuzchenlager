...
public class TokenExchangeService {
  ...
  public String exchangeTheOneTimeToken(final String clientId, final String clientSecret,
      final String oneTimeToken) {

    final String nonce = UUID.randomUUID().toString();

    OAuthClientCredentials clientCredentials = OAuthClientCredentials.create(clientId, clientSecret,
        nonce);

    try (Response response = authproviderRestClient.exchangeOneTimeTokenWithJwt(oneTimeToken,
        clientCredentials);) {
      ...
      String message =
          "Konnte das oneTimeToken nicht gegen das JWT tauschen: " + messagePayload.getMessage();
  ...
  private String checkNonceAndExtractTheJwt(final String expectedNonce,
    ...
    if (messagePayload.isOk()) {
      ...
      LOGGER.error("Authentisierung des Clients hat nicht geklappt: {} - {}",
