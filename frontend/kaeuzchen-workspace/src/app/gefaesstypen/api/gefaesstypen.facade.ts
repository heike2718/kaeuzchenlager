...
export class GefaesstypenFacade {
  ...
  readonly gefaesstypenLoaded$: Observable<boolean> = this.#store.select(
    fromGefaesstypen.selectGefaesstypenLoaded
  );
  readonly gefaesstypen$: Observable<Gefaesstyp[]> = this.#store.select(
    fromGefaesstypen.selectGefaesstypen
  );
  ...
  public selectGefaesstyp(gefaesstyp: Gefaesstyp): void {
    ...
    this.#store.dispatch(
      gefaesstypenActions.selectGefaesstyp({ uuid: gefaesstyp.uuid, navigateTo: navigateTo })
    );
  }
}
