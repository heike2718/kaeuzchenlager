...
public class CsrfTokenValidationFilter implements ContainerRequestFilter {
  ...
  private static final List<String> SECURE_HTTP_METHODS = Arrays.asList(
      new String[]{"OPTIONS", "GET", "HEAD"});

  /**
   * Erst nach dem login ist csrf-protection erforderlich. Weiter erst einmal keine Einschränkungen
   * auf Pfade.
   */
  ...
  public void filter(final ContainerRequestContext requestContext) throws IOException {
    ...
    if (!this.compareCookieAndHeader(requestContext)) {
      requestContext.abortWith(
          Response.status(403).entity(MessagePayload.error(INVALID_CSRF_RESPONSE_PAYLOD)).build());
    }

    if (!this.verifyCsrfTokenSignature(requestContext, sessionId)) {
      LOGGER.warn("Verifizierung der csrfTokenCookie-Signatur schlug fehl bei {} path={}", method,
          path);
      requestContext.abortWith(
          Response.status(403).entity(MessagePayload.error(INVALID_CSRF_RESPONSE_PAYLOD)).build());
    }
  ...
  boolean compareCookieAndHeader(ContainerRequestContext requestContext) {
    ...
    if (csrfTokenHeader == null || csrfTokenHeader.isEmpty()) {
      LOGGER.warn("csrfTokenHeader == null oder csrfTokenHeader.size() == 0 bei {} path={}", method,
          path);
      return false;
    }
    ...
    if (!identifyAsEquals(headerValue, csrfCookieValue)) {
      ...
      LOGGER.warn("cookieValue != headerValue: [headerValue={}, cookieValue={}, path={}",
          headerValue, csrfCookieValue, path);
      return false;
    }
